...
Developers *must* test everything they assume about the interaction with their contract, both succeeding and failing calls. The failure tests are just as important as the passing tests. 

Successful test suites are those that comprehensively cover *every possible interaction with a contract*.

## Prerequisites

- Completing [Battleship Compact tutorial](./smart-contract.mdx) 
- Yarn 

## Setup

First let's do some project setup. 
...
This config file is only setup for local devnet, but can be easily extended to support other network configurations by adding to this pattern.
...
This wallet class can be reused across different projects, it does not change significantly with different Compact contract interactions.
...
The providers are an important component of MidnightJS and they provide an interface for setting and retrieving data from private state, public data, wallet operations and more. We'll see more about how to use this in our test file.
...
Local devnet comes with pre-funded addresses for the first 3 seed accounts. We'll use Alice as one and Bob as two. Each user also needs a string for identifying their private state.
...
`getConfig()` returns the network configuration from our `config.ts` file and this repository is currently setup to operate on an `undeployed` network. The `const` assignments that follow are for placing each players ship locations uniquely.
...
Deploying the contract is the logical first thing to do, it sends our code out to the network and allows access through our circuit definitions via MidnightJS calls. In order to call the `deployContract` function of MidnightJS we need to create an initial private state for Alice the Deployer™:
...
When this call finalizes, it returns `deployed` of the `DeployedContract` type that can be inspected and verified itself.
...
We setup for Bob's private state in the same way we did for Alice, though this time the shape of our MidnightJS function has changed slightly. `submitCallTx` is the function we want to execute for basic contract calls and it has similar requirements to `deployContract`. Inspect it closely to check for differences. `submitCallTx` is what we'll use to interact with our contract now that it is deployed.
...
This pattern should start to look familiar and continue to be used for most of the remaining tests. But what about the pattern for failed transaction calls? After all, testing the failures is just as important as testing the successful calls. Let's add a cheat attempt by Bob to shoot out of turn:
```typescript
    it('Allows Alice to take the first shot(MISS)', async () => {
        const shot = BigInt(5);// miss
        ...
        await expect(async () => {// this is the new call
...
Now that you've seen the pattern for successful and failing calls, write some tests yourself that progress the interaction of the contract. Look back at the `.compact` code to verify the state after a certain circuit executes and check these through `queryLedger` at the end of each test.

To view the entire test suite for reference see `battleship.test.ts` in [example-battleship](https://github.com/midnightntwrk/example-battleship/blob/main/src/test/battleship.test.ts).

The linear progression of this tutorial has reached its end. Using the test file in the link above should provide the needed code for completing these tests. 
...
- Private state management (setting, getting and changing it)
- Explicit state management in our Compact code
- Calling in to our contract through MidnightJS
- Successful and failing contract calls
- Defensive programming

Thanks for following along. Shout out in the Discord dev-chat that you've become a Battleship master or share any feedback on this tutorial. 

To inspect the full Battleship repository, see [example-battleship](https://github.com/midnightntwrk/example-battleship).
