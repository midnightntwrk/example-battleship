...
`getConfig()` returns the network configuration from our `config.ts` file and this repository is currently setup to operate on an `undeployed` network. The `const` assignments that follow are for placing each players ship locations uniquely. These are assigned here for programmatic testing, but in the production version of this application these values would be presented by the user at some UI component.
...
Deploying the contract is the logical first thing to do, it sends our code out to the network and allows access to our contract circuits via MidnightJS calls. In order to call the `deployContract` function of MidnightJS we need to create an initial private state for Alice the Deployer™:
```typescript
    it('deploys the contract', async () => {
...
Local devnet requires a Node, Indexer and Proof server running locally, so be sure to leave these services running for the duration of these tests.
...
For a challenge, work with the returned `txData` from `submitCallTx` to inspect and operate on the data there.
